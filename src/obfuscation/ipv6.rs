...
static IMPORTS: &str = "use std::net::Ipv6Addr;";
...
pub fn deobfuscate(obfuscated: &Vec<String>) -> Vec<u8> {
    ...
    for ip_str in obfuscated.iter() {
        let ip: Ipv6Addr = ip_str.parse().expect("Invalid IP address format");
...
pub fn obfuscate(shellcode: &Vec<u8>) -> Vec<String> {
    let ipv6_vec = transform(shellcode);

    let mut obfuscated_shellcode = Vec::new();

    for ipv6 in ipv6_vec.iter() {
        obfuscated_shellcode.push(ipv6.to_string());
    }

    obfuscated_shellcode
}

pub fn transform(shellcode: &Vec<u8>) -> Vec<Ipv6Addr> {
